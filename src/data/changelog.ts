...
export const CHANGELOG: ChangelogEntry[] = [
  {
    date: '2026-08-20',
    items: [
      '選考フェーズの「辞退 / 不採用」を「見送り」（自社都合の不採用）と「選考辞退」（候補者都合の辞退）に分割。カンバンボードの列、候補者詳細・一覧のフェーズ選択、フィルター、ダッシュボードの集計グラフがそれぞれ2つに分かれて表示されるように',
      '「見送り」「選考辞退」に変更する際、理由メモを任意で入力できるように（候補者詳細に保存され、あとから確認可能）',
      '過去に登録された「辞退 / 不採用」の候補者は、自動的に「見送り」として引き継がれます'
    ]
  },
  {
    date: '2026-08-10',
    items: [
      ...
      '更新履歴（このモーダル）を追加',
      '複数人が同時にアプリを開いている場合、他の人による候補者の状況変更などが自動的に反映されるように（タブを開いたまま約20秒ごとに自動更新）',
      '選考評価フォームに「面接方式（対面／オンライン）」欄を追加（次回面接官の指定と同時に選択可能）',
      '書類選考通過スレッドへのChat通知に、選考ポジション・次回面接官・面接方式を記載するように',
      '評価確定時のChat通知（候補者スレッド）にも、選考ポジション・総合所感・次回の面接方式を記載するように',
      '管理者が先んじて手動登録した採用担当者と同じ氏名でGoogleログインした場合、新規レコードを作らず既存の登録情報に統合できるように（webhookや面接官アサインの齟齬を防止）',
      '評価保存直後、他の人の自動同期（20秒ごとのDrive確認）とタイミングが重なると、保存したばかりの選考フェーズ変更が古い状態に巻き戻ってしまうことがある不具合を修正',
      '書類選考を不合格にした際、本来は合格者のみに使う「書類選考通過スレッド」用の候補者スレッドが誤って作成されてしまう不具合を修正',
      '候補者詳細の「基本情報を編集」から、主担当者・書類選考担当者を変更できるように（これまで候補者詳細画面に変更手段がなく、選考サマリ等に最新のアサインが反映されなかった不具合を解消）',
      '面談ログ取り込みの権限エラーを、原因ごとに正しいメッセージが出るよう修正（面談に参加していないため議事録ファイルを開けない場合は、再ログインではなく参加者に取り込みを依頼するよう案内）',
      'タブを切り替えた際、直前のタブのスクロール位置が持ち越されて少し下にスクロールした状態で表示されてしまう不具合を修正',
      '候補者一覧テーブルで、在籍企業名などが長い場合に隣の列と文字が重なって読めなくなる不具合を修正',
      '候補者一覧テーブルで、辞退/不採用の候補者をデフォルトで非表示に変更（表示するボタンを追加）'
    ]
  },
  {
    date: '2026-08-09',
