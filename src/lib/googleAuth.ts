...
// calendar.readonly and gmail.readonly are both best-effort (not scope-gated like Drive below):
// they only power the "採用MTG" calendar/Gmail-linked import, and denying/skipping either shouldn't
// block sign-in or any of the Drive-dependent features the rest of the app actually needs to
// function. Note: an already-signed-in user's stored token won't pick up a newly-added scope until
// they sign out and back in (silent/background token refresh can't grant a scope that hasn't been
// interactively consented to) — see RecruitmentMeetingView's Gmail import button for the fallback
// message shown when that happens.
const SCOPES = 'openid email profile https://www.googleapis.com/auth/drive https://www.googleapis.com/auth/calendar.readonly https://www.googleapis.com/auth/gmail.readonly';
